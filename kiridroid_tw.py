...
if not os.path.exists(JAVA_BIN):
    raise RuntimeError(f"未找到內建Java: {JAVA_BIN}")
...
def replace_launcher_icons(res_dir, user_icon_path):
    ...
    for subdir in drawable_dirs:
        ...
        if os.path.exists(target_path):
            ...
            replaced += 1
    if replaced == 0:
        raise RuntimeError("找不到任何drawable-*-v4目錄，圖示替換失敗！")
    return replaced
    ...
    try:
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            try:
                with tempfile.TemporaryDirectory() as tmpdir_c:
                    ...
                    for abi, so_src in abi_map.items():
                        ...
                        if not os.path.exists(so_src):
                            play_sound("error.wav")
                            ui_updater.show_error("錯誤", f"找不到{so_src}，請檢查32/64位libc++_shared.so是否齊全")
                            ui_updater.set_status(f"缺少{abi}的libc++_shared.so")
                            ui_updater.enable_btn()
                            return
                    ...
                    for abi in abi_map.keys():
                        ...
                        if not os.path.exists(so_path):
                            play_sound("error.wav")
                            ui_updater.show_error("錯誤", f"ABI目錄 {abi} 缺少libc++_shared.so，已中斷打包！")
                ...
                ui_updater.enable_btn()
                return
            ui_updater.set_status("正在簽名APK...")
            ui_updater.step_progress(20)
            signed_apk = os.path.join(OUTPUT_DIR, f"{app_name}_signed.apk")
            if not os.path.exists(rebuilt_apk):
                play_sound("error.wav")
                ui_updater.show_error("錯誤", f"回包後的APK檔案不存在: {rebuilt_apk}")
                ui_updater.set_status("回包檔案不存在")
                ui_updater.enable_btn()
                return
            rebuilt_size = os.path.getsize(rebuilt_apk)
            if rebuilt_size < 1000000:
                play_sound("error.wav")
                ui_updater.show_error("錯誤", f"回包後的APK檔案可能損壞，大小: {rebuilt_size} 位元組")
                ui_updater.set_status("回包檔案損壞")
                ui_updater.enable_btn()
                return
            if APKSIGNER is None:
                play_sound("error.wav")
                ui_updater.show_error("錯誤", "找不到apksigner工具！請檢查build-tools目錄。")
                ui_updater.set_status("apksigner未找到")
                ui_updater.enable_btn()
                return
            ...
            if not os.path.exists(signed_apk):
                play_sound("error.wav")
                ui_updater.show_error("錯誤", "簽名後APK未產生！")
                ui_updater.set_status("簽名後APK遺失")
                ui_updater.enable_btn()
                return
            try:
                with zipfile.ZipFile(signed_apk, 'r') as zf:
                    test = zf.testzip()
                    if test is not None:
                        raise Exception(f"APK壓縮檔損壞: {test}")
            except Exception as e:
                play_sound("error.wav")
            ...
            try:
                ...
                logging.warning(f"自動開啟output資料夾失敗: {e}")
        ...
        for d in tmpdirs:
            try:
                ...
                pass
        logging.exception("打包流程發生例外")
        ui_updater.set_status("發生例外")
        play_sound("error.wav")
...
def main():
    ...
    tk.Label(main_frame, text="選擇遊戲資料夾：", font=("Microsoft JhengHei", 10)).grid(row=0, column=0, sticky="w", pady=5)
    ...
    tk.Label(main_frame, text="選擇圖示檔案：", font=("Microsoft JhengHei", 10)).grid(row=1, column=0, sticky="w", pady=5)
    ...
    tk.Label(main_frame, text="自訂套件名稱：", font=("Microsoft JhengHei", 10)).grid(row=2, column=0, sticky="w", pady=5)
    ...
    tk.Label(main_frame, text="自訂應用名稱：", font=("Microsoft JhengHei", 10)).grid(row=3, column=0, sticky="w", pady=5)
    ...
    tk.Label(main_frame, text="請至 output 資料夾查看結果。", font=("Microsoft JhengHei", 10)).grid(row=4, column=0, columnspan=3, sticky="w", pady=10)
